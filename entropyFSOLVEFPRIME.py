...
def f(p, data, params, factors, trips, sep, cost, model, constraints, knowns):
            data = balanceFactors(data, sep, cost, factors, constraints, model)
            data = estimateFlows(data, sep, cost, model, factors)
            estimates = estimateCum(data, knowns)
            return abs(np.sum(np.array(buildLLFunctions(p, data, params, factors, trips, sep, cost, model, constraints, knowns))))

def fprime(PV, data, params, factors, trips, sep, cost, model, constraints, knowns):
    functions = buildLLFunctions(PV, data, params, factors, trips, sep, cost, model, constraints, knowns)
    jac = []
    for function in functions:
        print function
        fJac = ndt.Jacobian(function)
...
def prodConstrain(observed, data, knowns, params, trips, sep, cost, factors, constraints):
    ...
    while abs(estimates - observed) > 10:
        paramSingle = []
        for param in params:
            if param != 'beta':
                paramSingle.append(data[str(param) + 'Param'].ix[0])
                ...
                paramSingle.append(data[param].ix[0])
        functions = buildLLFunctions(paramSingle, data, params, factors, trips, sep, cost, model, constraints, knowns)
        print functions

        #paramSingle = fmin(f, paramSingle, (data, params, factors, trips, sep, cost, model, constraints, knowns))
        #print paramSingle

        updates = fsolve(buildLLFunctions, paramSingle, (data, params, factors, trips, sep, cost, model, constraints, knowns), fprime=fprime)
        #print paramSingle
        #updates = new(paramSingle, data, params, sep, trips, functions)
        ...
        its += 1
        if (abs(paramSingle[0]- updates[0])/abs(paramSingle[0])) < .0000001:
            break
